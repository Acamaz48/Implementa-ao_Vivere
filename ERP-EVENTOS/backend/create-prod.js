...
async function bootstrapProd() {
  console.log('\n🚀 [BOOTSTRAP] Iniciando rotina de criação de Produtor...\n');

  // Captura os argumentos da linha de comando ou usa os valores padrão seguros
  const email = process.argv[2] || 'prod@vivere.com';
  const plainPassword = process.argv[3] || 'Vivere@Prod2026';
  const name = process.argv[4] || 'Produtor de Infraestrutura';
  ...
  try {
    ...
    console.log('\n✅ [SUCESSO] Usuário Produtor criado e validado!\n');
    console.log('====================================================');
